...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    user_id INTEGER, kind TEXT, amount INTEGER,
    note TEXT DEFAULT '', ts INTEGER
);
CREATE TABLE IF NOT EXISTS receipts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER, amount INTEGER, file_id TEXT,
    status TEXT DEFAULT 'pending',
    admin_id INTEGER DEFAULT 0, reason TEXT DEFAULT '',
    ts INTEGER, decided_at INTEGER DEFAULT 0
);
CREATE TABLE IF NOT EXISTS plans (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT, days INTEGER, limit_gb INTEGER,
    price INTEGER, is_active INTEGER DEFAULT 1, sort INTEGER DEFAULT 0
);
CREATE TABLE IF NOT EXISTS coupons (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    code TEXT UNIQUE, kind TEXT, value INTEGER,
    min_amount INTEGER DEFAULT 0, max_uses INTEGER DEFAULT 0,
    used INTEGER DEFAULT 0, expires_at INTEGER DEFAULT 0,
    is_active INTEGER DEFAULT 1
);
CREATE TABLE IF NOT EXISTS coupon_uses (
    coupon_id INTEGER, user_id INTEGER,
    PRIMARY KEY (coupon_id, user_id)
);
CREATE TABLE IF NOT EXISTS bot_accounts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER, plan_id INTEGER,
    email TEXT UNIQUE, sub_id TEXT,
    expires_at INTEGER, limit_bytes INTEGER, ts INTEGER
);
CREATE TABLE IF NOT EXISTS tickets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER, message TEXT,
    status TEXT DEFAULT 'open', ts INTEGER
);
...
def connect() -> sqlite3.Connection:
    ...
    with _lock:
        ...
        for k, v in DEFAULTS.items():
            _conn.execute("INSERT OR IGNORE INTO settings(k,v) VALUES(?,?)",
                          (k, v))
        _conn.commit()
        _seed_plans()
        return _conn
...
def _seed_plans():
    ...
    for title, days, gb, price, sort in plans:
        ex("INSERT INTO plans(title,days,limit_gb,price,is_active,sort) "
...
def set_setting(k, v):
    ex("INSERT INTO settings(k,v) VALUES(?,?) "
...
def ensure_user(tg_id: int, username: str = "") -> dict:
    ...
    if not u:
        ...
        while q("SELECT tg_id FROM users WHERE ref_code=?", (code,), one=True):
            code = "r" + secrets.token_hex(3)
        ex("INSERT INTO users(tg_id,username,ref_code,joined_at) "
           "VALUES(?,?,?,?)", (tg_id, username or "", code, now_ms()))
        u = q("SELECT * FROM users WHERE tg_id=?", (tg_id,), one=True)
    elif username and username != u["username"]:
...
def balance_add(tg_id: int, amount: int, kind: str, note: str = ""):
    ex("UPDATE users SET balance=balance+? WHERE tg_id=?",
       (amount, tg_id))
    ex("INSERT INTO transactions(user_id,kind,amount,note,ts) "
       "VALUES(?,?,?,?,?)", (tg_id, kind, amount, note, now_ms()))
...
def log_admin(admin_id: int, action: str, detail: str = ""):
    ex("INSERT INTO admin_log(admin_id,action,detail,ts) VALUES(?,?,?,?)",
...
def check_coupon(code: str, amount: int, tg_id: int):
    """→ (خطا | None, مبلغ تخفیف)"""
    c = q("SELECT * FROM coupons WHERE code=? AND is_active=1",
          (code.strip(),), one=True)
    if not c:
    ...
    if q("SELECT coupon_id FROM coupon_uses WHERE coupon_id=? AND user_id=?",
         (c["id"], tg_id), one=True):
        return "شما قبلاً از این کد استفاده کرده‌اید.", 0
    discount = (amount * c["value"] // 100) if c["kind"] == "percent" \
        else c["value"]
...
def coupon_commit(code: str, tg_id: int):
    c = q("SELECT id FROM coupons WHERE code=?", (code.strip(),), one=True)
    if not c:
        return
    ex("UPDATE coupons SET used=used+1 WHERE id=?", (c["id"],))
    ex("INSERT OR IGNORE INTO coupon_uses(coupon_id,user_id) VALUES(?,?)",
       (c["id"], tg_id))
