#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""پنل مدیریت ربات فروش — بخش ۵ از ۶ (v2 اصلاح‌شده)
نقش‌ها: owner (اولین آیدی tg_admins پنل) · admin (بقیه)
"""
...
from . import texts
from .bot import (send_message, answer_cbq, main_kb, _states, now_ms,
                  get_role)
...
def cmd_panel(chat):
    ...
    tg = int(chat)
    _states.pop(tg, None)
    pending = sdb.q("SELECT COUNT(*) n FROM receipts WHERE status='pending'",
                    one=True)["n"]
    users_n = sdb.q("SELECT COUNT(*) n FROM users", one=True)["n"]
    open_t = sdb.q("SELECT COUNT(*) n FROM tickets WHERE status='open'",
                   one=True)["n"]
    kb = {"inline_keyboard": [
...
def route_callback(cbq, data):
    ...
    try:
        if data == "adm:menu":
            ...
            answer_cbq(cbq_id)
            user_find(tg)
        elif p[1] == "user" and len(p) >= 3:
            answer_cbq(cbq_id)
            user_card(tg, int(p[2]))
        elif p[1] == "uchg" and len(p) >= 3:
            _states[tg] = {"step": STEP_ADMIN_INPUT,
                           "data": {"action": "chg", "uid": int(p[2])}}
            answer_cbq(cbq_id)
            send_message(tg, "💵 مبلغ (مثبت=شارژ، منفی=کسر) به تومان:\n"
                             "مثال: <code>50000</code> یا <code>-20000</code>")
        elif p[1] == "ublk" and len(p) >= 3:
            answer_cbq(cbq_id)
            user_block(tg, int(p[2]))
        elif p[1] == "umsg" and len(p) >= 3:
            _states[tg] = {"step": STEP_ADMIN_INPUT,
            ...
            _states[tg] = {"step": STEP_ADMIN_INPUT,
                           "data": {"action": "planadd"}}
            answer_cbq(cbq_id)
            send_message(tg, "📦 پلن جدید — ۴ خط:\n<code>عنوان\nروز\nگیگ\nقیمت</code>\n"
                             "مثال:\n<code>۱ ساله\n365\n200\n449000</code>")
        elif p[1] == "plantgl" and len(p) >= 3:
            answer_cbq(cbq_id)
            sdb.ex("UPDATE plans SET is_active=1-is_active WHERE id=?",
                   (int(p[2]),))
            plans_list(tg)
        elif p[1] == "plandel" and len(p) >= 3:
            ...
            _states[tg] = {"step": STEP_ADMIN_INPUT,
                           "data": {"action": "coupadd"}}
            answer_cbq(cbq_id)
            send_message(tg, "🎟 کد تخفیف — به این شکل:\n"
        ...
        elif p[1] == "couptgl" and len(p) >= 3:
            answer_cbq(cbq_id)
            sdb.ex("UPDATE coupons SET is_active=1-is_active WHERE id=?",
                   (int(p[2]),))
            coupons_list(tg)
        elif p[1] == "couprem" and len(p) >= 3:
            answer_cbq(cbq_id, "حذف شد")
            ...
            answer_cbq(cbq_id)
            settings_menu(tg)
        elif p[1] == "set" and len(p) >= 3:
            key = p[2]
            _states[tg] = {"step": STEP_ADMIN_INPUT,
                           "data": {"action": "set", "key": key}}
            answer_cbq(cbq_id)
            labels = {"card": "شماره کارت", "name": "نام صاحب حساب",
                      "ref": "درصد ریفرال", "tgb": "گیگ تست",
                      "tdays": "روز تست", "tcool": "فاصله تست (روز)",
                      "mindep": "حداقل شارژ", "title": "عنوان فروشگاه"}
            send_message(tg, f"⚙️ مقدار جدید «{labels.get(key, key)}» را بفرست:")
...
def rcps_list(tg: int):
    rows = sdb.q("SELECT * FROM receipts WHERE status='pending' "
                 "ORDER BY id LIMIT 15")
    if not rows:
        send_message(tg, "🧾 رسید در انتظاری نیست. ✅",
                     reply_markup=main_kb())
        return
    for r in rows:
        u = sdb.q("SELECT username FROM users WHERE tg_id=?",
                  (r["user_id"],), one=True)
        uname = ("@" + u["username"]) if u and u["username"] else "—"
        kb = {"inline_keyboard": [[
            {"text": "✅ تایید", "callback_data": f"adm:rcp:{r['id']}:ok"},
            {"text": "❌ رد", "callback_data": f"adm:rcp:{r['id']}:no"}]]}
        send_message(tg, (
            f"🧾 <b>رسید #{r['id']}</b>\n"
...
def rcp_approve(tg: int, rid: int, cbq_id=""):
    r = sdb.q("SELECT * FROM receipts WHERE id=? AND status='pending'",
              (rid,), one=True)
    if not r:
        answer_cbq(cbq_id, "قبلاً تعیین شده")
        return
    sdb.ex("UPDATE receipts SET status='approved', admin_id=?, "
           "decided_at=? WHERE id=?", (tg, now_ms(), rid))
    sdb.balance_add(r["user_id"], r["amount"], "deposit",
                    f"تایید رسید #{rid}")
    send_message(r["user_id"], texts.T["receipt_ok"].format(
        amount=r["amount"]))
    sdb.log_admin(tg, "receipt_approve",
                  f"#{rid} +{r['amount']} برای {r['user_id']}")
...
def _find_user(qry: str):
    ...
    if qry.lstrip("-").isdigit():
        return sdb.q("SELECT * FROM users WHERE tg_id=?",
                     (int(qry),), one=True)
    return sdb.q("SELECT * FROM users WHERE username=? COLLATE NOCASE",
                 (qry,), one=True)
...
def user_card(tg: int, uid: int):
    ...
    if not u:
        ...
        return
    buys = sdb.q("SELECT COUNT(*) n FROM bot_accounts WHERE user_id=?",
                 (uid,), one=True)["n"]
    accs = sdb.q("SELECT email FROM bot_accounts "
                 "WHERE user_id=? ORDER BY id DESC LIMIT 10", (uid,))
    accs_txt = "\n".join(f"  · <code>{_esc(a['email'])}</code>"
                         for a in accs) or "  —"
    refs = sdb.q("SELECT COUNT(*) n FROM users WHERE ref_by=?",
                 (uid,), one=True)["n"]
    kb = {"inline_keyboard": [
        [{"text": "💵 تغییر موجودی", "callback_data": f"adm:uchg:{uid}"},
         {"text": "✉️ پیام", "callback_data": f"adm:umsg:{uid}"}],
        [{"text": ("⛔ مسدود" if not u["is_blocked"] else "✅ رفع مسدود"),
          "callback_data": f"adm:ublk:{uid}"}],
        [{"text": "🔙 پنل", "callback_data": "adm:menu"}],
    ]}
    send_message(tg, (
        f"👤 <b>کاربر</b> <code>{uid}</code>"
        f" @{_esc(u['username'] or '—')}\n"
        f"💰 موجودی: <b>{_toman(u['balance'])}</b>\n"
        f"🛒 خرید: {buys} · 🎁 ریفرال: {refs}"
        f" (+{_toman(u['ref_earnings'])})\n"
        f"📅 عضویت: {time.strftime('%Y-%m-%d', time.localtime(u['joined_at']/1000))}\n"
        f"{'⛔ <b>مسدود</b>' if u['is_blocked'] else ''}\n"
        f"🔗 کد: <code>{u['ref_code']}</code>\n"
        f"📦 اکانت‌ها:\n{accs_txt}"), reply_markup=kb)


def user_block(tg: int, uid: int):
    u = sdb.q("SELECT is_blocked FROM users WHERE tg_id=?", (uid,), one=True)
    if not u:
        return
    ...
    send_message(uid, "⛔ شما توسط مدیر مسدود شدید."
                 if new else "✅ مسدودی شما برداشته شد.")
    user_card(tg, uid)
...
def plans_list(tg: int):
    ...
    for p in rows:
        kb = {"inline_keyboard": [
            [{"text": ("✅ فعال" if p["is_active"] else "🚫 غیرفعال"),
              "callback_data": f"adm:plantgl:{p['id']}"},
             {"text": "🗑", "callback_data": f"adm:plandel:{p['id']}"}]]}
        send_message(tg, (
            f"📦 <b>{_esc(p['title'])}</b>\n"
            f"⏳ {p['days']} روز · 💾 {p['limit_gb']} گیگ · "
            f"💰 {_toman(p['price'])}"), reply_markup=kb)
    send_message(tg, "➕ افزودن پلن جدید:",
                 reply_markup={"inline_keyboard": [[
                     {"text": "📦 پلن جدید", "callback_data": "adm:planadd"},
                     {"text": "🔙 پنل", "callback_data": "adm:menu"}]]})
...
def coupons_list(tg: int):
    ...
    for c in rows:
        kb = {"inline_keyboard": [[
            ...
            {"text": "🗑", "callback_data": f"adm:couprem:{c['id']}"}]]}
        val = (f"{c['value']}%") if c["kind"] == "percent" \
            else _toman(c["value"])
        exp = (time.strftime("%m-%d", time.localtime(c["expires_at"] / 1000))
               if c["expires_at"] else "∞")
        uses = (f"{c['used']}/{c['max_uses']}"
                if c["max_uses"] else str(c["used"]))
        send_message(tg, (
            f"🎟 <code>{_esc(c['code'])}</code> — {val}\n"
            f"حداقل خرید: {_toman(c['min_amount'])} · "
            f"استفاده: {uses} · انقضا: {exp}"),
            reply_markup=kb)
    send_message(tg, "➕ ساخت کد جدید:",
                 reply_markup={"inline_keyboard": [[
                     {"text": "🎟 کد جدید", "callback_data": "adm:coupadd"},
                     {"text": "🔙 پنل", "callback_data": "adm:menu"}]]})
...
def tickets_list(tg: int):
    rows = sdb.q("SELECT * FROM tickets WHERE status='open' "
                 "ORDER BY id DESC LIMIT 10")
    if not rows:
        send_message(tg, "💬 تیکت بازی نیست. ✅",
                     reply_markup=main_kb())
        return
    for t in rows:
        send_message(tg, (
            ...
            reply_markup={"inline_keyboard": [[
                {"text": "↩️ پاسخ", "callback_data": f"adm:tik:{t['id']}"}]]})
...
def settings_menu(tg: int):
    ...
    kb = {"inline_keyboard": [
        [{"text": "💳 شماره کارت", "callback_data": "adm:set:card"},
         {"text": "👤 نام صاحب حساب", "callback_data": "adm:set:name"}],
        [{"text": "🤝 درصد ریفرال", "callback_data": "adm:set:ref"},
         {"text": "🎁 گیگ تست", "callback_data": "adm:set:tgb"}],
        [{"text": "⏳ روز تست", "callback_data": "adm:set:tdays"},
         {"text": "🕐 فاصله تست (روز)", "callback_data": "adm:set:tcool"}],
        [{"text": "💵 حداقل شارژ", "callback_data": "adm:set:mindep"},
         {"text": "🏷 عنوان فروشگاه", "callback_data": "adm:set:title"}],
        [{"text": "🔙 پنل", "callback_data": "adm:menu"}],
    ]}
    send_message(tg, f"⚙️ <b>تنظیمات</b>\n\n💳 <code>{_esc(card)}</code>"
                     f" — {_esc(name)}", reply_markup=kb)
...
def set_value(tg: int, key: str, value: str):
    ...
    if key in ("ref", "tgb", "tdays", "tcool", "mindep"):
        if not v.isdigit():
            ...
            return
    sdb.set_setting(key, v)
    sdb.log_admin(tg, "setting", f"{key}={v[:50]}")
...
def stats(tg: int):
    deposits = sdb.q("SELECT COALESCE(SUM(amount),0) s, COUNT(*) n "
                     "FROM transactions WHERE kind='deposit'",
                     one=True)
    sales = sdb.q("SELECT COALESCE(SUM(-amount),0) s, COUNT(*) n "
                  "FROM transactions WHERE kind='purchase'", one=True)
    day_ms = now_ms() - 86400000
    today_sales = sdb.q("SELECT COALESCE(SUM(-amount),0) s, COUNT(*) n "
                        "FROM transactions WHERE kind='purchase' AND ts>?",
                        (day_ms,), one=True)
    users_n = sdb.q("SELECT COUNT(*) n FROM users", one=True)["n"]
    new_today = sdb.q("SELECT COUNT(*) n FROM users WHERE joined_at>?",
                      (day_ms,), one=True)["n"]
    top = sdb.q("SELECT p.title, COUNT(*) n FROM bot_accounts b "
...
def admin_log_view(tg: int):
    ...
    txt = "\n".join(
        ...
        for r in rows) or "—"
    send_message(tg, f"📜 <b>لاگ ادمین‌ها</b>\n{txt}",
                 reply_markup=main_kb())
...
def fsm_input(tg: int, text: str, states: dict, st, data) -> bool:
    ...
    if action == "find_user":
        ...
        if u:
            ...
            send_message(tg, "یافت نشد. آیدی عددی یا @یوزرنیم:")
        return True
    ...
    if action == "chg":
        ...
        try:
            ...
            return True
        u = sdb.q("SELECT balance FROM users WHERE tg_id=?", (uid,), one=True)
        if not u:
            states.pop(tg, None)
            return True
        ...
        send_message(uid, f"💵 موجودی شما به مقدار "
                          f"<b>{sign}{amount:,}</b> تومان تغییر کرد.")
        states.pop(tg, None)
        user_card(tg, uid)
        return True
    ...
    if action == "umsg":
        uid = data["uid"]
        send_message(uid, f"✉️ <b>پیام مدیر:</b>\n\n{_esc(text[:800])}")
        states.pop(tg, None)
    ...
    if action == "planadd":
        ...
        if len(lines) < 4 or not all(x.replace(",", "").isdigit()
                                     for x in lines[1:]):
            send_message(tg, "❌ ۴ خط: عنوان/روز/گیگ/قیمت (اعداد صحیح)")
            return True
        title, days, gb, price = (lines[0],
                                  int(lines[1].replace(",", "")),
                                  int(lines[2].replace(",", "")),
                                  int(lines[3].replace(",", "")))
        sort = 1 + (sdb.q("SELECT COALESCE(MAX(sort),0) s FROM plans",
                          one=True)["s"])
        sdb.ex("INSERT INTO plans(title,days,limit_gb,price,is_active,sort) "
               "VALUES(?,?,?,?,1,?)", (title, days, gb, price, sort))
    ...
    if action == "coupadd":
        ...
        if len(parts) < 3:
            send_message(tg, "❌ قالب: code|نوع|مقدار|حداقل|سقف|روز")
            return True
        code, kind = parts[0], parts[1].lower()
        ...
        if kind == "percent" and not (1 <= value <= 99):
            ...
            return True
        mn = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 0
        mx = int(parts[4]) if len(parts) > 4 and parts[4].isdigit() else 0
        dy = int(parts[5]) if len(parts) > 5 and parts[5].isdigit() else 0
        exp = now_ms() + dy * 86400000 if dy else 0
        if sdb.q("SELECT id FROM coupons WHERE code=?", (code,), one=True):
            send_message(tg, "❌ این کد قبلاً ساخته شده.")
            return True
        sdb.ex("INSERT INTO coupons(code,kind,value,min_amount,max_uses,"
               "expires_at,is_active) VALUES(?,?,?,?,?,?,1)",
               (code, kind, value, mn, mx, exp))
        sdb.log_admin(tg, "coupon_add", code)
        states.pop(tg, None)
        send_message(tg, f"✅ کد <code>{_esc(code)}</code> ساخته شد.",
                     reply_markup=main_kb())
        return True
